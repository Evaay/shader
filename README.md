...
Shader 1: Patrón Distorsionado con Control Interactivo
  El  objetivo de este shader fue implementar la interactividad con el usuario mediante la entrada del ratón.
...
Shader 2: Túnel Rotatorio con Ruido Dinámico
  Este shader se desarrolló a partir de un código base con efecto túnel.
  ...
  Se combinaron funciones smoothstep() para definir los bordes y sin() para introducir un patrón de ruido o líneas oscilantes.
  La función mix() se utilizó para interpolar entre dos colores (c1 y c2), ambos definidos por funciones trigonométricas que varían con u_time, asegurando un degradado de color cíclico y animado.

Shader 3: Patrón de Círculos Animado
  Se centró en la creación de patrones y diferenciación de comportamiento por celda.
  Se escalaron las coordenadas de la textura por un factor de 8.0, y se usaron floor() y fract() para aislar la coordenada de la celda de la coordenada local dentro de la celda.
